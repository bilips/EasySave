...
# 保存函数(自动加密)
func save_data_width_auto(path, game_data):
	var saver = File.new()
...
# 读取函数(解密)
func load_data_with_password(path, password):
	var loader = File.new()
...
# 保存函数(加密)
func save_with_password(path, game_data, password):
	var saver = File.new()
